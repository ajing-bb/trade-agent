...
## `LOC_ACADEMY_PLAYGROUND_DAY`

### Summary

- `status`: prompted
- `required_outputs`: `scene_master_day`、`reverse_seed`
...
## `LOC_HEADMASTER_OFFICE_DAY`

### Summary

- `status`: prompted
- `required_outputs`: `office_master_day`、`window_dialogue_reverse`
...
```text
headmaster office inside Shenfeng Academy, large window looking out to the academy sky, heavy desk, bookshelves or file wall, western god relief details, authority space for high-ranking school leader, daylight interior, clean spatial layout, clean comic linework, 2-step cel shading, 2D Chinese manhua still, not CEO office, not palace, not modern glass conference room --ar 16:9 --v 7 --raw
```

#### window_dialogue_reverse

```text
same academy headmaster office, reverse-side view of the same room, preserve large window, heavy desk, shelf wall and relief details, same daylight mood, readable father-daughter dialogue coverage, clean comic linework, 2-step cel shading, 2D Chinese manhua still --ar 16:9 --v 7 --raw --oref [HEADMASTER_OFFICE_MASTER] --ow 200
```
...
```text
保留校长办公室的大窗、办公桌、书柜墙和学院装饰浮雕不变，只清理错误家具和不稳定细节，使其成为司徒昆仑与司徒青青对话的稳定母图。不要改成企业总裁室。
```
...
- `status`: prompted
- `required_outputs`: `scene_master_night`、`skyline_reverse_seed`
