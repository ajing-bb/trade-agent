...
- 写入 `assets/BUG修不好但她偏偏喜欢我/项目档案/` 的确认版本，才作为续集、补镜头、重做资产时的默认依据。
...
- `episodes/ep001/`
  - 当前已初始化的标准化文本、拆解、连续性方案、导演队列、资产清单
- `progress-update-sop.md`
  - 每次工作结束前如何判断是否需要更新进度，以及更新顺序

## 当前约定

- 原始剧本来源：`assets/BUG修不好但她偏偏喜欢我/剧本/BUG修不好但她偏偏喜欢我.docx`
- 当前状态：`ep001 已初始化；series bible / prompt packs / director queue 已生成；资产状态仍以 planned 为主`
- `Midjourney` 只负责底图
