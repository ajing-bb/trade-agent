---
name: okx-dex-token
description: "Use this skill for token discovery and token-level analytics: search by name, symbol, or address; look up token metadata; see trending or ranked tokens; inspect holder distribution; or review market cap, liquidity, volume, and related token info across XLayer, Solana, Ethereum, Base, BSC, Arbitrum, Polygon, and 20+ other chains. Do not use for vague requests like 'tokens' or 'crypto' with no concrete question. For simple current price checks, charts, candlestick data, trade history, smart-money signals, or meme-token safety checks, use okx-dex-market instead."
license: Apache-2.0
metadata:
  author: okx
  version: "1.0.1"
