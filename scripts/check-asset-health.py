...
def check_scene_bible(archive_dir: Path, findings: list[dict[str, str]]) -> None:
...
def check_prop_vfx_bible(archive_dir: Path, findings: list[dict[str, str]]) -> None:
...
def check_asset_manifest(archive_dir: Path, findings: list[dict[str, str]]) -> dict[str, str]:
    mapping: dict[str, str] = {}
    ...
    for episode_dir in sorted(episodes_dir.glob("ep*")):
        ...
        for item in data.get("assets", []):
            ...
            if item_type == "character" and status == "committed" and raw_path:
                mapping[item_id] = raw_path
...
def check_cross_consistency(
    findings: list[dict[str, str]], manifest_characters: dict[str, str], bible_characters: dict[str, str]
) -> None:
    manifest_ids = set(manifest_characters)
    bible_ids = set(bible_characters)
    for item_id in sorted(manifest_ids | bible_ids):
        manifest_path = manifest_characters.get(item_id)
        bible_path = bible_characters.get(item_id)
        if manifest_path and not bible_path:
            add_finding(
                findings,
                "error",
                "missing_character_bible_entry",
                "cross-check",
                item_id,
                "asset_root",
                manifest_path,
                "committed character exists in asset-manifest but not in character-bible",
            )
            continue
        if bible_path and not manifest_path:
            add_finding(
                findings,
                "error",
                ...
                "path",
                bible_path,
                "committed character exists in character-bible but not in asset-manifest",
            )
            continue
        if manifest_path and bible_path:
            if resolve_record_path(manifest_path) != resolve_record_path(bible_path):
                add_finding(
                    ...
                    "path",
                    manifest_path,
                    "asset-manifest path and character-bible asset_root do not match",
                )
...
def main() -> None:
    ...
    check_scene_bible(archive_dir, findings)
    check_prop_vfx_bible(archive_dir, findings)
    manifest_characters = check_asset_manifest(archive_dir, findings)
    check_character_prompt_pack(archive_dir, findings)
    check_cross_consistency(findings, manifest_characters, bible_characters)
